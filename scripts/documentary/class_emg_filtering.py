...
class Reading_EMG:
    ...
    def __init__(self, path, filename, ids_channels):
        ...
        for num_ch in np.arange(ids_channels[0], ids_channels[1]+1):
            ...
            self.df_data[self.channelsNames[i]] = self.channels[i]
    ...
    def plotSignals(self, time_interval):
        ...
        for ch_n in self.channelsNames:
            ax[cont].plot(df[self.ch_time_name], df[ch_n], label=ch_n)
            ax[cont].legend()
            cont+=1
    ...
    def plotEnvelopedSignals(self, ids_emg, list_act_emg, channels_names, time_interval):
