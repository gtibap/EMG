...
def main(args):
    ...
    time_interval = [1010, 1020] ## 900 seconds are 15 min
    ...
    ## calculate RMS of the EMG signals per pedal cycle (per lap, turn, revolution)
    ...
    fig, ax = plt.subplots(nrows=2,ncols=1, figsize=(7,3.5), )
    fig.canvas.mpl_connect('key_press_event', on_press)
    sel_max = arr_time_max[0:20]
    sel_min = arr_time_min[0:20]
    signal_name = 'VLO LT, uV'
    right_side = False
    rms_list = obj_emg.plotFlexionExtension(ax[0], sel_max, sel_min, signal_name, right_side)
