...
def delta_mov(df, cg_rb_position, jg_rb_position):
    ...
    for cg,jg in zip(cg_rb_position, jg_rb_position):
        ...
        # print(f'some nan indices: {np.isnan(delta).any()}')
    
    # x = df.iloc[:,6].to_numpy()
    # y = df.iloc[:,7].to_numpy()
    # z = df.iloc[:,8].to_numpy()
    ...
    return  pos_cg, pos_jg, norm_delta


def main(args):
    
    filename = '../data/tracking_006_s1/e1.csv'
    ...
    cg_rb_rotation= list(range(ids[0],ids[0]+3))
    ...
    jg_rb_rotation= list(range(ids[6],ids[6]+3))
    ...
    norm_delta_d, yhat_d, norm_sub = delta_mov(df, jd_rb_position, jg_rb_position)
    norm_delta=np.linalg.norm(norm_delta_d, axis=0)
    norm_yhat=np.linalg.norm(yhat_d, axis=0)
    ...
    ax.plot(time,norm_delta, label='a')
    
    # ax.plot(time,yhat_d[0])
    # ax.plot(time,yhat_d[1])
    # ax.plot(time,yhat_d[2])
    ax.plot(time,norm_yhat+0.23, label='b')
